<template lang="pug">
.e-header.pos-f.fn-flex.flex-row
	h1 EslinkV
	ul.fn-flex.flex-row.e-header-nav
		li.pointer.pos-r(
			:class="{ active: item.url === common.navIndex }",
			@click="handleLink(item.url, item.title)",
			v-for="item in list",
			:key="item.title") {{ item.title }}
	i-button(type="primary", v-if="!common.user", @click="handleLogin") 登录
	img.circle.e-header-user-avatar(:src="userAvatar", v-if="common.user")
	i-drop-down.e-header-user(@on-click="handleUser", v-if="common.user")
		.pointer
			span.e-header-user-name {{ name }}
			i-icon(type="ios-arrow-down", color="#fff")
		i-drop-down-menu(slot="list")
			i-drop-down-item(name="child", v-if="!common.user.userIsChild")
				i-icon(type="md-people", :size="16", color="#333")
				span.e-header-user-item 子账号管理
			i-drop-down-item(name="updatePassword")
				i-icon(type="ios-cog", :size="16", color="#333")
				span.e-header-user-item 修改登录密码
			i-drop-down-item(name="secretKey")
				i-icon(type="md-lock", :size="16", color="#333")
				span.e-header-user-item 密钥管理
			i-drop-down-item(name="link")
				i-icon(type="ios-link", :size="16", color="#333")
				span.e-header-user-item 外链管理
			i-drop-down-item(name="logout")
				i-icon(type="md-log-out", :size="16", color="#333")
				span.e-header-user-item 退出登录
	i-modal(
		v-model="updatePasswordModalVisible",
        title="修改登录密码",
		:mask-closable="false",)
			Form(@keydown.native.enter.prevent="updatePassword", autocomplete="off")
				FormItem(label="旧密码")
					i-input(autocomplete="new-password", prefix="md-contact", placeholder="请输入旧密码", v-model.trim="oldPassword")
				FormItem(label="新密码")
					i-input(
						autocomplete="new-password",
						prefix="md-lock",
						placeholder="请输入新密码",
						v-model.trim="newPassword",
						type="password")
				FormItem(label="再次输入新密码")
					i-input(
						autocomplete="new-password",
						prefix="md-lock",
						placeholder="请再次输入新密码",
						type="password"
						v-model.trim="newPasswordRepeat",
						@on-blur="validatePasswordRepeat")
			template(#footer)
				i-button(@click="updatePasswordModalVisible = false") 取消
				i-button(type="primary" @click="updatePassword") 确认
			
</template>
<script lang="ts">
import { Vue, Component } from 'vue-property-decorator'
import {
	Icon,
	Button,
	Dropdown,
	DropdownItem,
	DropdownMenu,
	Modal,
	Form,
	FormItem,
	Input,
} from 'view-design'
import common from '../../store/common.store.js'
import { logout, detail, update } from '@/api/user.api.js'

@Component({
	components: {
		'i-icon': Icon,
		'i-button': Button,
		'i-drop-down': Dropdown,
		'i-drop-down-item': DropdownItem,
		'i-drop-down-menu': DropdownMenu,
		'i-modal': Modal,
		'i-input': Input,
		Form,
		FormItem,
	},
})
export default class EHeader extends Vue {
	common = common.state
	updatePasswordModalVisible = false
	oldPassword = ''
	newPassword = ''
	newPasswordRepeat = ''
	userAvatar =
		'data:image/png;base64,'
	index = ''
	list = [
		{
			url: '/editor/Manger',
			title: '大屏管理',
		},
		// {
		// 	url: '/market/componentList',
		// 	title: '媒体资源',
		// },
		{
			url: '/market',
			title: '组件开发',
		},
		{
			url: '/help/EslinkV',
			title: '帮助中心',
		},
	]

	get name(): string {
		if (this.common.user) {
			return this.common.user.nickName || this.common.user.userName
		} else {
			return ''
		}
	}

	handleLogin(): void {
		this.$router.push('/login')
	}

	handleUser(name: string): void {
		switch (name) {
			case 'logout':
				common.actions.setUser(null)
				localStorage.removeItem('eslinkv-login')
				logout()
				this.$router.push('/login')
				break
			case 'child':
				this.$router.push('/userChild')
				break
			case 'secretKey':
				this.$router.push('/secretKey')
				break
			case 'updatePassword':
				this.updatePasswordModalVisible = true
				break
			case 'link':
				this.$router.push('/link')
				break
		}
	}

	handleLink(url: string, title: string): void {
		common.actions.setNavIndex(url)
		document.title = title
		this.$router.push(url)
	}

	validatePasswordRepeat(): boolean {
		const valid = this.newPassword === this.newPasswordRepeat
		if (!valid) {
			this.$Message.error('新密码两次输入不一致')
		}
		return valid
	}

	async updatePassword(): Promise<void> {
		if (
			[
				this.newPassword,
				this.newPasswordRepeat,
				this.oldPassword,
			].includes('')
		) {
			this.$Message.error('请完善表单后再提交！')
			return
		}
		if (this.validatePasswordRepeat()) {
			await update({
				newPassword: this.newPassword,
				oldPassword: this.oldPassword,
			})
			this.$Message.success('密码更改成功')
			this.updatePasswordModalVisible = false
		}
	}

	mounted(): void {
		common.actions.setNavIndex(this.$route.path)
		detail()
			.then(res => {
				common.actions.setUser(res)
			})
			.catch(() => {
				common.state.user = null
				localStorage.removeItem('eslinkv-login')
				window.top.location.href = `${location.origin}/login`
			})
	}
}
</script>
<style lang="scss" scoped>
.e-header-user {
	height: 100%;
}
.e-header-user-item {
	margin-left: 10px;
}

.e-header-user-avatar {
	width: 30px;
	height: 30px;
}

.e-header-user-name {
	margin-left: 8px;
	color: #fff;
	line-height: 50px;
	margin-right: 10px;
}

.e-header-nav {
	height: 100%;
	margin-right: auto;

	li {
		height: 100%;
		padding: 0 14px;
		font-size: 14px;
		line-height: 50px;
		color: rgb(191, 191, 191);

		&:hover {
			background-color: rgb(48, 50, 62);
		}

		&.active {
			background-color: rgb(48, 50, 62);

			&::before {
				position: absolute;
				bottom: 0;
				left: 0;
				width: 100%;
				height: 3px;
				content: '';
				background-color: var(--themeColor);
			}
		}
	}
}

.e-header {
	top: 0;
	left: 0;
	z-index: 8;
	align-items: center;
	width: 100%;
	height: 50px;
	padding: 0 20px;
	background-color: #22242b;

	h1 {
		width: 200px;
		font-size: 18px;
		color: #fff;
	}
}
</style>
