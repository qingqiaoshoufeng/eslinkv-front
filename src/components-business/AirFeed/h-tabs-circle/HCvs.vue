		...
		renderDesc() {
			this.source &&
				this.source.forEach((item, index) => {
					...
					this.drawText(
						countStartX,
						countStartY - countInterval * index,
						fontColor,
						countStyle,
						countText,
					)
					...
					const unitStartX =
						countStartX +
						this.getTextWith(countText, countStyle) +
						8
					// 单位y轴
					...
					// 百分比x轴
					const percentStartX =
						unitStartX + this.getTextWith('km', unitStyle) + 24
					// 百分比y轴
					...
					this.drawText(
						percentStartX,
						percentStartY - percentInterval * index,
						fontColor,
						percentStyle,
						`${percent}%`,
					)
