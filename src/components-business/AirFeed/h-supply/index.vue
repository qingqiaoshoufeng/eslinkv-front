		...
		<div class="container">
			<div class="main">
				<ul>
					...
					>
						<div class="title">
							{{ k.name }} {{ k.value | toThousand }}
						</div>
						<div
							class="bar"
							:style="{ width: getBarWidth(k.value) }"
						></div>
					</li>
				</ul>
			...
			<div class="outside">
				...
				<div class="sub">
					<div
						class="sub-item"
						v-for="(k, i) in subItem || curr.children || []"
						:key="i"
					>
						<div class="sub-item-rank font-num">
							{{ getIndex(i) }}
						</div>
						<div class="sub-detail">
							...
							<div
								class="sub-bar"
								:style="{ width: getSubBarWidth(k.value) }"
							></div>
						</div>
					</div>
				</div>
		...
		startInterval() {
			this.timer = setInterval(() => {
				if (inside) {
					if (
						inside ===
						Math.ceil(this.data[this.index].children.length / SIZE)
					) {
						inside = 0
					} else {
						inside++
						this.subItem = this.data[this.index].children.slice(
							SIZE * (inside - 1),
							SIZE * inside,
						)
						return
