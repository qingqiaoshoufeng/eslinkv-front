<template>
	<div class="widget-part" :style="styles">
		<div class="s-progress-1">
			<ul>
				<li
					class="fn-flex flex-row pos-r"
					v-for="(item, index) in config.config.background"
					:key="index"
				>
					<i
						:style="{
							backgroundImage: `url(${
								config.config &&
								config.config.background[index].background
							})`,
						}"
					></i>
					<label>
						{{
							config.config &&
							config.config.background[index].title
						}}
					</label>
					<div class="fn-flex flex-row s-progress-1-line-box pos-r">
						<div
							class="s-progress-1-line"
							v-for="(child, key) in icons"
							:key="key"
						></div>
					</div>
		...
		data: {
			handler(val) {
				if (val) {
					...
					val.forEach(item => {
						realArr.push(
							new Int8Array(Math.ceil((item.value / max) * 50)),
						)
					})
		...
		gradientColors(start, end, steps, gamma) {
			...
			for (i = 0; i < steps; i++) {
				...
				for (j = 0; j < 3; j++) {
					so[j] = this.pad(
						Math.round(
							Math.pow(start[j] * me + end[j] * ms, 1 / gamma) *
								255,
						).toString(16),
					)
				}
