<template>
	<div class="widget-part" :style="styles" v-if="data">
		<div class="tab">
			<div class="swiper" @click="back" :class="{ active: offset > 0 }">
			...
			<div
				class="swiper"
				@click="next"
				:class="{ active: offset < list.length }"
			>
...
export default {
	...
	methods: {
		setOption(data) {
			this.instance &&
				this.instance.setOption(getOption(data, this.config.config))
		},
		back() {
			...
			handler(val) {
				if (val) {
					if (val.compareType === 'year') {
						...
						this.config.config.lineName2 =
							new Date().getFullYear() - 1
					}
					if (val.compareType === 'month') {
						this.config.config.lineName1 = `${
							new Date().getMonth() + 1
						}月`
