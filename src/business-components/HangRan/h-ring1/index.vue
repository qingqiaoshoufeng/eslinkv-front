	...
	<div class="widget-part pos-r" :style="styles">
		...
		<div class="pos-a h-ring-1-legend-box">
			<ul class="h-ring-1-legend" :style="legengdTransform">
				<li class="fn-flex flex-row" v-for="(item,index) in data?data.data:[]" :key="index"
					:class="[{active:animateActiveIndex===index}]">
					<i class="circle" :style="{color:color[index%color.length]}"/>
...
<script>
	...
	export default {
		...
		computed: {
			legengdTransform() {
				if (this.data) {
					if (this.data.data.length <= this.showSize + 1) {
...
<style lang="scss">
	...
	.h-ring-1-icon {
		background-image: url('/static/icons/h-ring1-1.png');
