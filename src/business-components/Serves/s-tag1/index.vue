<template>
	<div class="widget-part" :style="styles" v-if="data">
		...
		<div class="txt">{{ config.config.desc }}</div>
    <div class="flex">
      <div>
        <div class="percent font-num">{{ data.percent }}%</div>
        <div class="percent-txt">{{ config.config.desc2 }}</div>
      </div>
      <div class="chart" :id="id" :style="{ backgroundImage: `url(${config.config.logo})` }"></div>
    </div>
	</div>
</template>
<script>
