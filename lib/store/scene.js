...
const actions = {
	...
	destroyScene(index) {
		...
		setTimeout(() => {
			...
			document.getElementsByClassName('scene-temporary-container')[0].remove()
			...
			store.scene.showAnimationStyle = 'zoom' // 初始化实例场景
		}, 300)
	},
	deleteScene(index) {
	...
	createSceneInstance(id, showAnimationStyle = 'zoom') {
