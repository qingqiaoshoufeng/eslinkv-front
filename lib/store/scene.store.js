...
const actions = {
	...
	listToObj(value) {
		...
		const scene = state.list
		widgets.forEach(item => {
			const index = scene.indexOf(item.scene)
			if (index !== -1) {
				if (!state.sceneObj[scene[index]])
					state.sceneObj[scene[index]] = {}
				if (!state.sceneObj[scene[index]].list)
					state.sceneObj[scene[index]].list = []
				state.sceneObj[scene[index]].list.push(item)
			}
		})
	},
	initScene(list) {
