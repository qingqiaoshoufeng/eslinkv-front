...
export default {
    ...
    methods: {
        ...
        desensitizeRegexFn() {
            // 脱敏正则。如果存在，开始脱敏
            if (this.data.desensitizeRegex) {
                let desensitizeEnName = ESVCommon.desensitize(
                    this.data.desensitizeEnName,
        ...
        handlerRules(template) {
            return util.handlerRules(template);
        },
        setDefaultBool(targetValue, defaultValue) {
            return ESVCommon.setDefaultBool(targetValue, defaultValue);
        },
        setDefaultBoolOrObject(targetValue, defaultValue) {
            if (typeof targetValue === 'boolean') {
                return targetValue;
            } else if (typeof targetValue === 'object') {
                if (ESVCommon.isEmptyObject(targetValue)) {
