<template>
    <div class="es-form-new" ref="esFormNew">
        <Form
            ...
            :class="{
                w100: true,
                enabledTip: !ESVCommon.setDefaultBool(data.disabledTip, true),
                disabledTip: ESVCommon.setDefaultBool(data.disabledTip, true),
            ...
            <slot name="templetDetailList">
                <template v-for="(templet, index) in data.templetDetailList">
                    ...
                    <es-form-item
                        ...
                        :editFlag="editFlag"
                        :disabledTip="
                            ESVCommon.setDefaultBool(data.disabledTip, true)
                        "
                    >
    ...
    watch: {
        height: {
            handler(value) {
                if (value) {
                    this.$nextTick(() => {
                        this.$refs.esFormNew.style.height = this.ESVCommon.toLength(
                            value
                        );
                    });
                }
        ...
        width: {
            handler(value) {
                if (value) {
                    this.$nextTick(() => {
                        this.$refs.esFormNew.style.width = this.ESVCommon.toLength(
                            value
                        );
                    });
                }
