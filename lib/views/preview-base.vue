  ...
  <div ref="canvas-wrapper" id="kanban" class="canvas-wrapper" :style="canvasStyle">
    ...
    <div :class="{ active: refilling }" class="refill-mask">
      <div class="mask-content">
        ...
        <Progress :percent="refillPercent" :stroke-width="20" text-inside :stroke-color="['#108ee9', '#87d068']" />
      </div>
    </div>
    <template v-for="item in widgetsAdded">
      <template v-if="!item.config.widget.combinationTo">
        <parts :key="item.id" :type="item.type" :config="item.config" :style="item.config.widget.hide ? 'display: none' : ''" readonly>
    ...
    <api-executor v-for="api in apis" :key="api.variable" :api="api" @api-data-update="(data) => handleApiDataUpdate(api.variable, data)" />
  </div>
</template>
<script>
import parts from './core/widgets/parts'
