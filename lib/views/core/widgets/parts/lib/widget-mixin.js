import {valueParser} from './config-parser'
...
const mixins = {
	...
	methods: {
		 ...
		 *
		 */
		parseConfigValue(configParseMap = {}, localConfigValue = null, customConfig) {
	...
	watch: {
		...
		'config.widget.componentVersion': {
			handler: function (value, oldValue) {
				// console.warn('111111112222233333')
				// if (!this.configReady) return
				// if (!/\d+\.\d+\.\d+/.test(value)) return
				// // 切换版本后，重新加载新版本
				// if (!window.GoldChart.components[`${this.config.widget.name}-${value}`]) {
				...
				// 			this.$parent.componentVersion = value
				// 		}
