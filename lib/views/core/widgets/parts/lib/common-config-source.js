...
const font = Object.freeze({
	...
	fields: {
		...
		lineHeight: getNumberWithUnit('行高', 'lineHeight', 'lineHeightUnit', ['无', 'px', 'em', 'rem'], {
			placeholder: '大小',
			step: 0.1
		}),
...
const padding = Object.freeze({
	...
	detail: {
		...
		fields: {
			...
			right: getNumberWithUnit('右边距', 'right', 'rightUnit', ['px', 'em', 'rem', '%'], {
				placeholder: '大小',
				min: 0
			}),
			bottom: getNumberWithUnit('下边距', 'bottom', 'bottomUnit', ['px', 'em', 'rem', '%'], {
				placeholder: '大小',
				min: 0
			})
		}
...
const widgetCommonConfigSource = Object.freeze({
	widget: {
		...
		fields: {
			...
			componentVersion: getInput('componentVersion', '版本号'),
	...
	api: {
		...
		system: {
			...
			fields: {
				...
				interface: getInput('interface', '接口地址', {defaultValue: '/api/impalaByQueryId'}),
				method: getSelect('method', '请求方式', ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'], {
					prepend,
					defaultValue: 'POST'
				}),
		...
		globalApiConfig: {
			...
			fields: {
				...
				variables: getDragDropPond('variables', '候选变量', {
					removeOnDragOut: false,
					labelKey: '__self',
					valueKey: '__self'
				}),
				variable: getDragDropPond('variable', '绑定变量', {
		...
		dataShareConfig: {
			...
			fields: {
				...
				useVariable: getDragDropPond('useVariable', '绑定名称', {
					showDeleteButton: true,
					max: 1,
					labelKey: '__self',
					valueKey: '__self'
