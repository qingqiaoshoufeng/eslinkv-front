...
export default {
  ...
  methods: {
    ...
    innerQuery (api) {
      ...
      this.$api.databaseQuery(params, method, innerUrl).then(response => {
    ...
    handleApiChange () {
      if (this.isBindParamsValidated && !this.bindFirstFetchOccurred) return
      const api = this.config.api
      if (!api) return
      if (this.isUseShareDataSource) return
    ...
    startAutoFetch () {
      this.stopAutoFetch()
      if (this.queryTimer || this.isBindParamsValidated && !this.bindFirstFetchOccurred) {
  ...
  computed: {
    apiChangeWatcher () {
      const { url, params, method, path, check = {}, process = {}, system = {}, dimensionAndQuota = {} } = this.config.api || {}
      ...
      const { enable: isDimensionAndQuotaEnabled, index: dataIndex, dimensions, quotas } = dimensionAndQuota
      return {
        ...
        isDimensionAndQuotaEnabled, dataIndex, dimensions, quotas
      }
    },
    autoFetchApi () {
      const api = this.config.api
    ...
    isUseShareDataSource () {
      return this.globalApiConfig && this.globalApiConfig.enable || this.isShareDataUser
    }
  },
  watch: {
    querying (value) {
