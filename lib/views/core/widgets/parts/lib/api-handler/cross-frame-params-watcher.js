...
export default {
  ...
  methods: {
    handleCrossFrameParam (params) {
      ...
      this.config.api.params = Object.keys(paramSet).length ? { ...paramSet } : ''
      params.forEach(crossParam => {
    ...
    handleCrossFrameParamUpdate (param, value) {
      ...
      this.config.api.params = { ...paramSet, [param]: value }
    }
  },
  computed: {
    apiParams () {
      return this.config.api ? parseParams(this.config.api.params) : null
    },
