...
export default {
  ...
  methods: {
    ...
    parseQueryResult (response, { path, process }) {
      const shareProcessedData = this.dataShareConfig ? this.dataShareConfig.shareProcessedData : null
      if (this.isShareDataProvider && shareProcessedData === '1') {
        this.dataForShare = response.headers && response.config ? response.data : response
      }
      response = this.usePath(path, response)
      if (this.isShareDataProvider && shareProcessedData === '2') {
        this.dataForShare = response
      }
      response = this.useProcess(process, response)
      response = this.useDimensionQuota(response)
      if (this.isShareDataProvider && shareProcessedData === '3') {
        this.dataForShare = response
      }
      this.data = response
    },
