<template>
  <div class="global-api-panel" @click.stop>
    <header class="header">
      <div>全局数据源配置</div>
      <div class="add-api action-button" @click="addApiConfig"><Icon type="md-add" size="18" /></div>
    </header>
    <div class="config-panel-wrapper">
      <template v-if="apis.length">
        <div v-for="(api, index) in apis" :key="index" class="config-panel-item">
          ...
          />
          <div class="remove-item action-button" @click="removeApiConfig(index)"><Icon type="md-close" size="16" /></div>
        </div>
      </template>
