...
export default {
  ...
  methods: {
    ...
    updateWidgetParamsBind () {
      ...
      this.bindTimer = setTimeout(() => {
        const bindSetting = this.api ? this.api.bind : null
    ...
    executeBindChange (params) {
      ...
      this.paramsUpdateTimer = setTimeout(() => {
        const paramSet = parseParams(this.api.params)
        ...
        this.api.params = { ...paramSet, ...params }
  ...
  computed: {
    ...
    isBindResponder () {
      return this.isBindEnabled && this.api.bind.role.includes('responder')
    },
    isBindParamsValidated () {
      if (!this.isBindResponder) return false
      const bindParams = this.api.bind.params
