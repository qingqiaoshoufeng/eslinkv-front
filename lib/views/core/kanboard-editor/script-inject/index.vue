<template>
  <div class="script-inject" @click.stop>
    <header class="header">
      <div>脚本嵌入管理</div>
      <div class="add-script action-button" @click="addScript"><Icon type="md-add" size="18" /></div>
    </header>
    <div class="config-panel-wrapper">
      <template v-if="scripts.length">
        <div v-for="(script, index) in scripts" :key="script.name" class="config-panel-item">
          ...
          />
          <div class="remove-item action-button" @click="removeScriptConfig(index)"><Icon type="md-close" size="16" /></div>
        </div>
      </template>
