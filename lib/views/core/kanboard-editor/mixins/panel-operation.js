export default {
	...
	methods: {
		...
		updatePanelPosition() {
			...
			this.$nextTick(() => {
				const ele = this.currentPartEle
				if (!ele) return null
				const {left, top, right, bottom} = ele
				const {clientWidth, clientHeight} = document.body
				let {width, height} = this.currentToolsMenuEle
				width += 10
				height += 10
	...
	computed: {
		...
		currentToolsMenuEle() {
			return document.getElementById('tools-menu').getBoundingClientRect()
		},
		currentPartEle() {
