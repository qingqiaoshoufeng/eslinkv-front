...
export default {
	data() {
		return {
			...
			canvasConfigValue: store.kanboard.panelConfig,
	...
	methods: {
		...
		updateRulerCanvas() {
			...
			this.rulerCanvasUpdateTimer = requestAnimationFrame(() => {
				const {width, height} = this.canvasConfigValue.size
				width && height && this.$refs.rulerCanvas.init()
				this.rulerCanvasUpdateTimer = null
				this.canvasConfigValue.size.preset = `${width}×${height}`
			})
		},
		updateCanvasOffset() {
			if (!this.$el) return
			...
			const {width: w, height: h} = this.canvasConfigValue.size
	...
	computed: {
		...
		canvasStyle() {
			return styleParser(this.canvasConfigValue)
		},
		canvasSize() {
			const {width, height} = this.canvasConfigValue.size
			return {width, height}
	...
	watch: {
		...
		'canvasConfigValue.size.width'() {
			this.updateRulerCanvas()
		},
		'canvasConfigValue.size.height'() {
			this.updateRulerCanvas()
		},
		'canvasConfigValue.size.preset'(value) {
			if (value === undefined) return
			if (value) {
				const [width, height] = value.split('×')
				if (width !== 'null') this.canvasConfigValue.size.width = +width
				if (height !== 'null') this.canvasConfigValue.size.height = +height
			} else {
				this.canvasConfigValue.size.width = 1920
				this.canvasConfigValue.size.height = 1080
			}
		}
	},
	mounted() {
		this.updateCanvasOffset()
		this.canvasConfigValue.info.name = !this.isTemplateEditor ? '看板1' : '模板1'
		setTimeout(() => {
