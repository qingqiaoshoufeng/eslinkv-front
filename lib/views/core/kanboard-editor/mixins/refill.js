export default {
	...
	methods: {
		...
		refillConfig() {
			const {kanboard, widgets, grids, apis, guides} = window.GoldChart.store.kanboard.data
			return new Promise(resolve => {
				...
				const widgetsArray = widgets ? this.sortWidgets(Object.values(widgets)) : []
				const length = widgetsArray.length
				// 小工具初始化需要时间，此处进行延时逐个回填
				const reDrawWidget = ({id, type, value, scene = 0, market = false}) => {
					this.initWidgetConfig(id, type, scene, market)
