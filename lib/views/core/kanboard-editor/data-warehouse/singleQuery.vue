<template>
  ...
  <div class="single-query">
    ...
    <div v-if="dataType == 0">
      <Form :model="querySingle" :label-width="90" class="single-query-form" ref="querySingle" :rules="querySingleRules" label-position="left">
        <FormItem label="项目名" prop="projectId" :label-width="70">
          <Select v-model="querySingle.projectId" @on-change="changeSinglePro" @on-open-change="openSinglePro" transfer filterable>
            <Option v-for="v in querySingleList.projectList" :value="v.value" :key="v.value">{{ v.label }}</Option>
          </Select>
        </FormItem>
        <FormItem label="数据库名称" prop="databaseId">
          <Select v-model="querySingle.databaseId" @on-change="changeSingleDatabase" @on-open-change="openSingleDatabase" transfer filterable>
            <Option v-for="v in querySingleList.databaseList" :value="v.value" :key="v.value">{{ v.label }}</Option>
          </Select>
        </FormItem>
        <FormItem label="表名称" prop="tableId" :label-width="70">
          <Select v-model="querySingle.tableId" @on-change="changeSingleTable" @on-open-change="openSingleTable" transfer filterable>
            <Option v-for="v in querySingleList.tableList" :value="v.value" :key="v.value">{{ v.label }}</Option>
          </Select>
        </FormItem>
        <FormItem :label="index > 0 ? '' : '条件'" :label-width="40" v-for="(item, index) in querySingle.cond.itemList" :key="index">
          <Row :gutter="4" :style="index == 0 ? 'transform:translateX(-2px)' : ''">
            <i-col :span="item.fieldCond == 'between' ? '4' : '6'" v-if="index > 0">
              <i-select v-model="item.operator" transfer filterable>
                <i-option v-for="v in querySingleList.condList" :value="v.value" :key="v.value">{{ v.label }}</i-option>
              </i-select>
            </i-col>
            <i-col :span="item.fieldCond == 'between' ? '3' : '5'">
              <i-select v-model="item.fieldName" transfer @on-change="changeFieldName(null, item.fieldName, index)" filterable>
                <i-option v-for="v in querySingleList.fieldList" :value="v.value" :key="v.value">{{ v.label }}</i-option>
              </i-select>
            </i-col>
            <i-col :span="item.fieldCond == 'between' ? '4' : '6'">
              <i-select v-model="item.fieldCond" transfer filterable>
                <i-option v-for="v in querySingleList.fieldCond" :value="v.value" :key="v.value">{{ v.label }}</i-option>
              </i-select>
            </i-col>
            ...
            <i-col span="2" style="display: flex">
              ...
              <Icon type="md-remove-circle" @click="removeSingleCond(index)" class="btn" v-if="querySingle.cond.itemList.length > 1" />
            </i-col>
          </Row>
        </FormItem>
      </Form>
