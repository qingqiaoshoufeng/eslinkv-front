<template>
  <Form :model="queryCustomSource" :label-width="70" style="width: 100%; display: block" ref="queryCustomSource" :rules="querySourceRule">
    <FormItem label="数据源" prop="dataSourceId" style="width: 100%">
      <Select v-model="queryCustomSource.dataSourceId" @on-open-change="openCustomPro" transfer filterable>
        <Option v-for="item in querySourceList.sourceList" :value="item.id" :key="item.id">{{ item.name }}</Option>
      </Select>
    </FormItem>
    ...
    <FormItem class="editor" label :label-width="0" style="height: 160px" prop="executeSql">
      ...
      <div class="es-sql-editor">
        <es-sql sqlName="custom-query" v-model="queryCustomSource.executeSql" @blur="sqlBlur" class="es-sql-box"></es-sql>
      </div>
    </FormItem>
  </Form>
</template>
...
export default {
  ...
  methods: {
    ...
    openBaseSchema () {
      this.$api.getSchemaList({ id: this.queryCustomSource.dataSourceId, databaseName: this.queryCustomSource.databaseName }).then((res) => {
    ...
    getProList () {
      ...
      let that = this;
      that.$api.getSourceList().then((res) => {
        if (res.responseCode == 100000) {
          let data = res.result;
    ...
    getDatabaseList (id) {
      ...
      this.$api.getSourceDatabaseList({ id: this.queryCustomSource.dataSourceId }).then((res) => {
        if (res.responseCode == 100000) {
          let data = res.result;
          if (data.length == 0) {
