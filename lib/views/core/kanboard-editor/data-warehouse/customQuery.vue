<template>
  ...
  <div class="custom-query">
    ...
    <Form v-if="dataType == 0" :model="queryCustom" :label-width="70" style="width: 100%; display: block" ref="queryCustom" :rules="queryCustomRule">
      <FormItem label="项目" prop="projectId" style="width: 100%">
        <Select v-model="queryCustom.projectId" transfer filterable @on-open-change="openCustomPro" @on-change="changeCustomPro">
          <Option v-for="v in queryCustomList.projectList" :value="v.value" :key="v.value">{{ v.label }}</Option>
        </Select>
      </FormItem>
      <FormItem label="数据库" prop="databaseId" style="width: 100%">
        <Select v-model="queryCustom.databaseId" transfer filterable @on-open-change="openCustomDatabase">
          <Option v-for="v in queryCustomList.databaseList" :value="v.value" :key="v.value">{{ v.label }}</Option>
        </Select>
      </FormItem>
