<template>
  ...
  >
    <FormItem :style="{ width: type == 1 ? '100%' : '' }" prop="dataSourceId" label="数据源" :label-width="80">
      <i-select v-model="queryCond.dataSourceId" :style="{ width: type == 0 ? '140px' : '100%' }" @on-open-change="getProList" filterable>
    ...
    <FormItem :style="{ width: type == 1 ? '100%' : '' }" v-if="databaseType == '1'" prop="databaseSchema" label="模式" :label-width="90">
      <i-select v-model="queryCond.databaseSchema" :style="{ width: type == 0 ? '140px' : '100%' }" filterable>
        <i-option v-for="item in dataSchemaeList" :value="item.value" :key="item.value">{{ item.value }}</i-option>
      </i-select>
    </FormItem>
      ...
      <Row :gutter="10" :style="index == 0 ? 'transform:translateX(-5px)' : ''">
        <i-col :span="type == 0 ? 2 : 5">
          <i-select v-model="item.operator" v-if="index > 0" transfer filterable>
            <i-option v-for="item in querylogicList" :value="item.value" :key="item.value">{{ item.label }}</i-option>
          </i-select>
        </i-col>
        <i-col :span="type == 0 ? 2 : 5">
          <i-select v-model="item.fieldName" clearable transfer @on-change="changeFieldName(null, item.fieldName, index)" filterable>
            <i-option v-for="item in queryFieldList" :value="item.value" :key="item.value">{{ item.label }}</i-option>
          </i-select>
        </i-col>
        <i-col :span="type == 0 ? 2 : 4">
          <i-select v-model="item.fieldCond" transfer filterable>
            <i-option v-for="item in queryCondList" :value="item.value" :key="item.value">{{ item.label }}</i-option>
          </i-select>
        </i-col>
        ...
        <i-col span="4">
          ...
          <Icon type="md-remove-circle" @click="searchCondRemove(index)" class="btn" v-show="queryCond.analyseCondition.length > 1" />
        </i-col>
      </Row>
    </FormItem>
    <FormItem prop="limitNum" :class="{ chart: type == 1 }" label="条数限制" :label-width="64" v-if="type == 0">
      <div style="display: flex">
        ...
        <i-select v-model="queryCond.limitNum" v-if="queryCond.isLimit">
          <i-option v-for="item in queryCond.limitNumList" :value="item.value" :key="item.value">{{ item.label }}</i-option>
        </i-select>
      </div>
    </FormItem>
  </Form>
...
export default {
  ...
  methods: {
    ...
    reShowChart (row) {
      ...
      this.isEcho = true
      this.singleCond = { databaseName: row.databaseName, tableName: row.tableName, databaseSchema: row.databaseSchema }
    },
    //数据是否回显
    cEcho (e) {
      this.isEcho = e
    ...
    getProList (bool) {
      this.isEcho = false
      if (bool) {
        this.$api.getSourceList().then((res) => {
    ...
    'queryCond.dataSourceId': {
      handler (value, oldValue) {
        if (value) {
          ...
          this.$api.getSourceDatabaseList({ id: value }).then((res) => {
            if (res.responseCode == 100000) {
              let list = []
              let data = res.result
              if (data.length == 0) {
      ...
      handler (value, oldValue) {
        if (value) {
          ...
          if (this.databaseType == 0) {
            this.$api.getSourceTableList({ id: this.queryCond.dataSourceId, databaseName: value }).then((res) => {
              if (res.responseCode == 100000) {
                let list = []
                let data = res.result
                if (data.length == 0) {
            ...
            this.queryCond.databaseSchema = ''
            this.$api.getSchemaList({ id: this.queryCond.dataSourceId, databaseName: value }).then((res) => {
              if (res.responseCode == 100000) {
    ...
    'queryCond.databaseSchema': {
      handler (value) {
        if (value && this.queryCond.databaseName) {
          ...
          this.tableList = []
          this.queryFieldList = []
          this.$api.getSourceTableList({ id: this.queryCond.dataSourceId, databaseName: this.queryCond.databaseName, databaseSchema: value }).then((res) => {
            if (res.responseCode == 100000) {
              let list = []
              let data = res.result
              if (data.length == 0) {
    ...
    'queryCond.tableName': {
      handler (value) {
        if (value) {
          ...
          let data
          if (this.databaseType == 0) {
            data = { id: this.queryCond.dataSourceId, databaseName: this.queryCond.databaseName, tableName: value }
          } else {
            data = {
          ...
          this.$api.getSourceTableDetail(data).then((res) => {
            if (res.responseCode == 100000) {
              let list = []
              let data = res.result
              data.map((item) => {
