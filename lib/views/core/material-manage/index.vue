<template>
  ...
  >
    <div slot="body" class="modal-body">
      <ul class="group">
        <li v-for="{ label, group } in categories" :key="group" :class="{ active: group === currnetGroup }" class="item" @click="currnetGroup = group">
          {{ label }}
      ...
      <div class="material-list">
        <ul ref="list" class="materials">
          ...
          <li v-for="item in materials" :key="item.id" class="item">
            ...
            <div v-show="currnetGroup === '-1'" class="group-name">{{ categories.find((group) => group.group === item.group).label }}</div>
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$api.deleteMaterial({ id }).then(() => {
            this.$api.deleteFile.post({ ossUrl: src }).then(() => { }).catch(() => {
